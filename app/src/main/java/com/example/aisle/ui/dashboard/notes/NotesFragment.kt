        ...
        viewModel.getNotesResult.observe(viewLifecycleOwner, Observer { response ->
            when (response) {
                is Resource.Success -> {
                    ...
                    _binding?.tvProfileInfo?.text = response.data?.invites?.profiles?.get(0)?.general_information?.first_name + ", " + response.data?.invites?.profiles?.get(0)?.general_information?.age
                }
                is Resource.Error -> {
                }
                is Resource.Loading -> {
